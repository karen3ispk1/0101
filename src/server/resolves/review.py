from src.database.models import Review as Model
...
def create(new: Model) -> int | dict:
    res = dbmanager.execute_query(
        query=f"insert into {Model.__name__} (name, age, staffID) values(?,?,?) returning id",
        args=(new.name, new.age, new.staff_id))
...
def update(id_: int, new: Model) -> None:
    return dbmanager.execute_query(
        query=f"update {Model.__name__} set (name, age, staffID) values(?,?,?) where id=(?)",
        args=(new.name, new.age, new.staff_id, id_))
