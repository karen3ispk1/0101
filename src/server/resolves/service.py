from src.database.models import Service as Model
...
def create(new: Model) -> int | dict:
    res = dbmanager.execute_query(
        query=f"insert into {Model.__name__} (name, price, duration) values(?,?,?) returning id",
        args=(new.name, new.price, new.duration))
...
def update(id_: int, new: Model) -> None:
    return dbmanager.execute_query(
        query=f"update {Model.__name__} set (name, price, duration) values(?,?,?) where id=(?)",
        args=(new.name, new.price, new.duration, id_))
